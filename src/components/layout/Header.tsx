...
export default function Header() {
  ...
  const handleShopLink = (href: string) => {
    router.push(href);
    if (href.startsWith("/shop")) router.refresh();
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden lg:flex items-center gap-8">
              <button
                onClick={() => handleShopLink("/shop")}
                className="text-xs font-medium tracking-widest uppercase transition-colors text-neutral-400 hover:text-white"
              >
                Shop
              </button>
              {categories.map((cat) => (
                <button
                  key={cat.id}
                  onClick={() => handleShopLink(`/shop?category=${cat.slug}`)}
                  className="text-xs font-medium tracking-widest uppercase transition-colors text-neutral-400 hover:text-white"
                >
                  {cat.name}
                </button>
              ...
              {profile ? (
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-full mt-1 w-48 bg-neutral-900 border border-white/10 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                    <Link href="/account" className="block px-4 py-2 text-sm text-neutral-300 hover:text-white hover:bg-white/5">
                      Minha Conta
                    </Link>
                    <Link href="/account/orders" className="block px-4 py-2 text-sm text-neutral-300 hover:text-white hover:bg-white/5">
                      Meus Pedidos
                    </Link>
                    {profile.role === "admin" && (
                      <Link href="/admin" className="block px-4 py-2 text-sm text-neutral-300 hover:text-white hover:bg-white/5">
                        Admin
                      </Link>
                    )}
                    <hr className="my-1 border-white/10" />
                    <button onClick={signOut} className="w-full text-left px-4 py-2 text-sm text-neutral-400 hover:text-white hover:bg-white/5">
                      Sair
                    </button>
                  </div>
                </div>
              ) : (
                <Link href="/login" className="p-2 text-neutral-400 hover:text-white transition-colors">
                  <User size={20} />
                </Link>
              )}
      ...
      {isMobileOpen && (
        <div className="fixed inset-0 z-40 bg-black pt-16">
          <nav className="flex flex-col px-6 py-8 gap-6">
            <button
              onClick={() => { handleShopLink("/shop"); setIsMobileOpen(false); }}
              className="text-2xl font-bold tracking-widest uppercase text-white hover:text-neutral-400 transition-colors text-left"
            >
              Shop
            </button>
            {categories.map((cat) => (
              <button
                key={cat.id}
                onClick={() => { handleShopLink(`/shop?category=${cat.slug}`); setIsMobileOpen(false); }}
                className="text-2xl font-bold tracking-widest uppercase text-white hover:text-neutral-400 transition-colors text-left"
              >
                {cat.name}
              </button>
