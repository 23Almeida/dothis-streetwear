...
async function getFooterSettings() {
  try {
    ...
    const { data } = await (supabase as any)
      ...
      .select("key, value")
      .in("key", ["brand", "social"]);
    if (!data) return { brand: defaultSettings.brand, social: defaultSettings.social };
    const brand = { ...defaultSettings.brand, ...(data.find((r: any) => r.key === "brand")?.value || {}) };
    const social = { ...defaultSettings.social, ...(data.find((r: any) => r.key === "social")?.value || {}) };
    return { brand, social };
  } catch {
    return { brand: defaultSettings.brand, social: defaultSettings.social };
  }
}
...
export default async function Footer() {
  const { brand, social } = await getFooterSettings();
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <div className="flex gap-4 mt-6">
              ...
              {!social.instagram && !social.twitter && !social.youtube && !social.tiktok && (
                <p className="text-xs text-neutral-700">Configure as redes sociais no painel admin.</p>
              )}
            </div>
          </div>
